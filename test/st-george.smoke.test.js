...
import { parseStGeorgeCsvToImportTransactions } from "../src/st-george.js";
...
function requiredEnv(name) {
  ...
  if (!value) {
    throw new Error(`${name} is required when running the St.George smoke test.`);
  }
  return value;
}
...
function sortTransactions(transactions) {
  return [...transactions].sort((left, right) =>
    `${left.date}|${left.imported_id}`.localeCompare(
      `${right.date}|${right.imported_id}`,
    ),
  );
}

async function runSmokeCase(t, { write }) {
  const fixturePath = process.env.ABCTL_SMOKE_FIXTURE ?? DEFAULT_FIXTURE_PATH;
  const budgetPrefix = process.env.ABCTL_SMOKE_BUDGET_PREFIX ?? "abctl-st-george-smoke";
  const accountName = process.env.ABCTL_SMOKE_ACCOUNT_NAME ?? "St George Smoke Test";
  ...
  const dataDir = await mkdtemp(path.join(os.tmpdir(), "abctl-st-george-"));
  ...
  const transactions = parseStGeorgeCsvToImportTransactions(csvText, { accountId });
  ...
  if (write) {
    ...
    for (let index = 0; index < actualView.length; index += 1) {
      ...
      assert.equal(actualTransaction.imported_id, expectedTransaction.imported_id);
...
test(
  "St.George smoke import dry run",
  {
    concurrency: false,
